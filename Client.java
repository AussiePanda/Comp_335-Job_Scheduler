...
public class Client
{
    ...
    String[] BFFServer;
	HashMap<String,List<Int>> simpleServers = new HashMap<String,<Int>>();
...
//        }
       ...
       // openServer(inputArrayString);
		sendNudes();
        String first = "SCHD " + inputArrayString[2]+" " + server +" "+Integer.toString(number) + "\n";
        try {
	...
	public void nootnoot (){
		...
		basic.add(0);
		basic.add(0);
		basic.add(ogServers.get(0)[4]);
		simpleServers.put(ogServers.get(0)[0],basic);
		int added = 0;
		
		for(int i=1;i<ogServers.get(ogServers.size()-1){
			for(String j : simpleServers.keySet()){
				if(ogServers.get(i)[0]==j){
					Helper = simpleServers.get(j);
					Helper.set(0,Helper[0]+1);
					simpleServers.put(j,Helper);
					added++;
				}
			}
			if(added==0){
				simpleServers.put(ogServers.get(i)[0],basic);
			}
			added=0;
		}
	...
	public void sendNudes(){
		...
		for(String i : simpleServers.keySet()){
			if(simpleServers.get(i).get(3)>inputArrayString[3]{
				server = i;
				number = simpleServer.get(i)[1]%simpleServer.get(i)[0];
				Helper = simpleServer.get(i);
				Helper[1]=Helper[1]+1;
				simpleServer.put(i,Helper);
				break;
			}			
		}
	}
...
/*    public void bestFit(ArrayList<String[]> sList) {
    ...
    public void schedule() {
        ...
        try{
            ...
            if(job.equals("JOBN")) {
                if (sortType.equals("noot")) {
                    nootnoot();
                    schd = "SCHD " + inputArrayString[2] + " " + server + " " + Integer.toString(number) + "\n";
                } else {
