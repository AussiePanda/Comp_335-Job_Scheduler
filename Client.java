...
{ 
 ...
 public void sort(){
	ArrayList<String[]> yess = new ArrayList<String[]>();

	yess.add(serverList.get(0));
	for(int i=1;i<serverList.size()-1;i++){
		for(int j = 0;j>=yess.size()-1;j++){
			if(yess.get(j)[4] >serverList.get(i)[4]){
				yess.add(j,serverList.get(i));	
				j=yess.size();		
			}
			else if(yess.size()-1==j){
				yess.add(serverList.get(j));
			}
		}
	}
